function [Q_aux_tot, dry_time, M_prod_fin, MR_fin, Irradiacion, irr_diag] = opt_tunel_mod2_v19_eta_sensitivity(m_max,T_min,r_div2,t_rec_ini, W0, m_i, Mi, mwi, md, m_f, Mf, mwf, M_des, mode_operation, eta_mode_v96z)
...
if nargin < 15 || isempty(eta_mode_v96z)
    eta_mode_v96z = "eta_constant_0p50";
end

mode_operation = string(mode_operation);
eta_mode_v96z = string(eta_mode_v96z);

if ~any(mode_operation == ["gasLP","hybrid","solar"])
    error("HYBRID-IRR-MODE: mode_operation must be gasLP, hybrid or solar.");
end

if ~any(eta_mode_v96z == ["eta_constant_0p50","eta_historical_code_curve","eta_article_2SAH_curve"])
    error("ETA-MODE-v96z: eta_mode_v96z must be eta_constant_0p50, eta_historical_code_curve, or eta_article_2SAH_curve.");
end

irr_diag = struct();
irr_diag.mode_operation = mode_operation;
irr_diag.eta_mode_v96z = eta_mode_v96z;
irr_diag.rule = "";
irr_diag.aux_rule = "";
...
for tiempo=t_ini:t_step:t_max
    ...
    G = I(i)*1000;  % Mantiene la escala usada por el código original
    
    if G <= 0 || ~isfinite(G)
        ETHA_capt(i) = 0;
    else
        switch eta_mode_v96z
            case "eta_constant_0p50"
                ETHA_capt(i) = 50/100;
                
            case "eta_historical_code_curve"
                P = 3.9;
                ETHA_capt(i) = 0.656 - 3.48050*(P/G) - 0.02380*(P^2/G);
                
            case "eta_article_2SAH_curve"
                % Sensibilidad inicial basada en curva de 2 SAHs.
                % P se mantiene provisionalmente igual a 3.9 para aislar el efecto
                % de coeficientes antes de acoplar temperatura dinámica.
                P = 3.9;
                ETHA_capt(i) = 0.570 - 5.4470*(P/G) - 0.0107*(P^2/G);
        end
        
        ETHA_capt(i) = max(0,min(0.75,ETHA_capt(i)));
    end
    
    irr_diag.ETHA_capt_values(end+1,1) = ETHA_capt(i);
