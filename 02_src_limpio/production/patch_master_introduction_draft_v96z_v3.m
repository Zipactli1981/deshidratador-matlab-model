%% patch_master_introduction_draft_v96z_v3.m
% 9.6z-draft-b
...
% Normalize draft block to a scalar character vector before applying contains/logical guards.
introBlock = char(join(string(introBlock), ''));
...
has2SAH_Draft = any(contains(string(introBlock), '2-SAH'));
hasSensitivity_Draft = any(contains(lower(string(introBlock)), 'sensitivity'));
Tchecks = addCheck(Tchecks, 'MID-DRAFT-03', 'Draft Introduction contains 2-SAH sensitivity framing', ...
    has2SAH_Draft && hasSensitivity_Draft, ...
    sprintf('2SAH=%d sensitivity=%d', has2SAH_Draft, hasSensitivity_Draft));

hasR1_7_Draft = any(contains(string(introBlock),'R1_solution_7'));
hasR1_3_Draft = any(contains(string(introBlock),'R1_solution_3'));
hasR1_9_Draft = any(contains(string(introBlock),'R1_solution_9'));
hasH2_Draft   = any(contains(string(introBlock),'H2'));
Tchecks = addCheck(Tchecks, 'MID-DRAFT-04', 'Draft Introduction contains candidate roles', ...
    all([hasR1_7_Draft, hasR1_3_Draft, hasR1_9_Draft, hasH2_Draft]), ...
    sprintf('R1_7=%d R1_3=%d R1_9=%d H2=%d', hasR1_7_Draft, hasR1_3_Draft, hasR1_9_Draft, hasH2_Draft));
