function cons = consolidate_guarded_postrun_evidence_v630b()
% CONSOLIDATE_GUARDED_POSTRUN_EVIDENCE_v630b
% Micropaso 6.30b — fix robusto para columnas importadas por readtable.
%
% Corrige el error:
%   Unrecognized table variable name 'mode'
%
% No modifica v10.
% No modifica v611.
% No repite AG.
% No sobrescribe corrida productiva.
    ...
    if ~isempty(missingRequired)
        disp(Tfiles)
        error('Faltan archivos indispensables para consolidar v630b: %s', strjoin(missingRequired,", "));
    end

    Tsel       = local_read_csv_v630b(files.selected_solution);
    T626main   = local_read_csv_v630b(files.v626_main);
    T626viol   = local_read_csv_v630b(files.v626_viol);
    T627rules  = local_read_csv_v630b(files.v627_rules);
    T628direct = local_read_csv_v630b(files.v628b_direct);
    T628obj    = local_read_csv_v630b(files.v628b_obj);
    T629full   = local_read_csv_v630b(files.v629_full);
    T629report = local_read_csv_v630b(files.v629_report);

    if isfile(files.v625_main)
        T625main = local_read_csv_v630b(files.v625_main);
    else
        T625main = table();
    end

    if isfile(files.v625_vars)
        T625vars = local_read_csv_v630b(files.v625_vars);
    else
        T625vars = table();
    end

    % Normalizar columnas clave
    T626main   = local_ensure_mode_column_v630b(T626main,   "T626main");
    T628direct = local_ensure_mode_column_v630b(T628direct, "T628direct");
    T628obj    = local_ensure_mode_column_v630b(T628obj,    "T628obj");
    T629full   = local_ensure_mode_column_v630b(T629full,   "T629full");
    T629report = local_ensure_mode_column_v630b(T629report, "T629report");

    T629report = local_ensure_column_alias_v630b(T629report, "cost_USD_per_kgwater", ["cost_USD_per_kgwater","cost_objective","cost_specific_USD_per_kgwater"]);
    T629report = local_ensure_column_alias_v630b(T629report, "dry_time_h", ["dry_time_h","dry_time"]);
    T629report = local_ensure_column_alias_v630b(T629report, "M_final", ["M_final","M"]);

    T626main = local_ensure_column_alias_v630b(T626main, "first_violation_i", ["first_violation_i"]);
    T626main = local_ensure_column_alias_v630b(T626main, "first_violation_variable", ["first_violation_variable"]);
    T626main = local_ensure_column_alias_v630b(T626main, "first_violation_rule", ["first_violation_rule"]);
    T626main = local_ensure_column_alias_v630b(T626main, "first_violation_value_real", ["first_violation_value_real"]);
    T626main = local_ensure_column_alias_v630b(T626main, "break_i", ["break_i"]);
    T626main = local_ensure_column_alias_v630b(T626main, "num_violations", ["num_violations"]);

    T628direct = local_ensure_column_alias_v630b(T628direct, "irr_status", ["irr_status"]);
    T628direct = local_ensure_column_alias_v630b(T628direct, "guard_i", ["guard_i"]);
    T628direct = local_ensure_column_alias_v630b(T628direct, "guard_variable", ["guard_variable"]);
    T628direct = local_ensure_column_alias_v630b(T628direct, "guard_rule", ["guard_rule"]);
    T628direct = local_ensure_column_alias_v630b(T628direct, "guard_value_real", ["guard_value_real"]);
    ...
    if isempty(gas629) || isempty(hyb629) || isempty(sol629)
        disp(T629report)
        error('T629report no contiene gasLP, hybrid y solar después de normalizar columna mode.');
    end
    ...
    if isempty(solar626) || isempty(gas626) || isempty(hyb626)
        error('T626main no contiene gasLP, hybrid y solar.');
    end

    if isempty(solar628) || isempty(gas628) || isempty(hyb628)
    ...
    outCsvSummary = fullfile(tablesDir,'GUARDED_POSTRUN_EVIDENCE_CONSOLIDATED_v630b_summary.csv');
    outCsvFiles   = fullfile(tablesDir,'GUARDED_POSTRUN_EVIDENCE_CONSOLIDATED_v630b_files.csv');
    outMat        = fullfile(matDir,'GUARDED_POSTRUN_EVIDENCE_CONSOLIDATED_v630b.mat');
    outMd         = fullfile(logsDir,'GUARDED_POSTRUN_EVIDENCE_CONSOLIDATED_v630b.md');
    outTxt        = fullfile(logsDir,'GUARDED_POSTRUN_EVIDENCE_CONSOLIDATED_v630b.txt');
    ...
    fprintf(fid,'# Micropaso 6.30b — CONSOLIDATE-GUARDED-POSTRUN-EVIDENCE-001\n\n');
    fprintf(fid,'## Estatus\n\n');
    ...
    for r = 1:height(Tfiles)
        fprintf(fid,'- `%s`: `%s` — exists = `%d`\n', string(Tfiles.key(r)), string(Tfiles.path(r)), Tfiles.exists(r));
    end
    ...
    disp('=== GUARDED_POSTRUN_EVIDENCE_CONSOLIDATED_v630b ===')
    disp(cons.status)
    disp('=== DIAGNOSIS ===')
    disp(cons.diagnosis)
    disp('=== FLAGS ===')
...
function T = local_read_csv_v630b(pathFile)
    try
        T = readtable(pathFile,'VariableNamingRule','preserve');
    catch
        T = readtable(pathFile);
    end

    T = local_sanitize_variable_names_v630b(T);
end

function T = local_sanitize_variable_names_v630b(T)
    oldNames = string(T.Properties.VariableNames);
    newNames = oldNames;

    for i = 1:numel(newNames)
        s = newNames(i);
        s = erase(s, char(65279));   % BOM
        s = strtrim(s);
        s = regexprep(s,'^"+|"+$','');
        ...
        if ~isletter(char(extractBefore(s,2)))
            s = "x_" + s;
        end
    ...
    newNames = matlab.lang.makeUniqueStrings(cellstr(newNames));
    T.Properties.VariableNames = newNames;
end

function T = local_ensure_mode_column_v630b(T, tableName)
    names = string(T.Properties.VariableNames);

    idx = find(strcmpi(names,"mode"),1);
    if ~isempty(idx)
        T.Properties.VariableNames{idx} = 'mode';
    ...
    idx = find(contains(lower(names),"mode"),1);
    if ~isempty(idx)
        T.Properties.VariableNames{idx} = 'mode';
        T.mode = string(T.mode);
        T.mode = erase(T.mode,'"');
    ...
    % Buscar una columna que contenga gasLP/hybrid/solar
    for j = 1:width(T)
        col = T{:,j};
        try
            scol = string(col);
            scol = erase(scol,'"');
            scol = strtrim(scol);
            if any(strcmp(scol,"gasLP")) && any(strcmp(scol,"hybrid")) && any(strcmp(scol,"solar"))
                T.Properties.VariableNames{j} = 'mode';
                T.mode = scol;
                return
            end
        catch
    ...
    disp("=== Variable names in " + tableName + " ===")
    disp(T.Properties.VariableNames)
    disp(T(1:min(5,height(T)),:))
    error('No se pudo identificar columna mode en %s.', tableName);
end

function T = local_ensure_column_alias_v630b(T, canonicalName, aliases)
    names = string(T.Properties.VariableNames);

    idx = find(strcmp(names,canonicalName),1);
    if ~isempty(idx)
        return
    end
    ...
    for i = 1:numel(aliases)
        a = aliases(i);
        idx = find(strcmpi(names,a),1);
        if ~isempty(idx)
            T.Properties.VariableNames{idx} = char(canonicalName);
            return
        end
    end
    ...
    for i = 1:numel(aliases)
        a = lower(string(aliases(i)));
        idx = find(contains(lower(names),a),1);
        if ~isempty(idx)
            T.Properties.VariableNames{idx} = char(canonicalName);
            return
        end
    ...
    disp("=== Missing column ===")
    disp("canonicalName = " + canonicalName)
    disp("aliases = ")
    disp(aliases)
    disp("available names = ")
    disp(T.Properties.VariableNames)
    error('No se pudo crear alias de columna: %s', canonicalName);
end