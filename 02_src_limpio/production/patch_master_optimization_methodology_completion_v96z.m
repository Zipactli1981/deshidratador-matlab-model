%% patch_master_optimization_methodology_completion_v96z.m
% 9.6z-draft-e
...
Tchecks = addCheck(Tchecks, 'MOMC-PRE-06', 'Optimization methodology currently marked PARTIAL', ...
    contains(string(optSeg), '`STATUS: PARTIAL`'), ...
    sprintf('present=%d', contains(string(optSeg), '`STATUS: PARTIAL`')));

Tchecks = addCheck(Tchecks, 'MOMC-PRE-07', 'Optimization methodology contains Expected content scaffold', ...
    contains(string(optSeg), 'Expected content:'), ...
    sprintf('present=%d', contains(string(optSeg), 'Expected content:')));

Tchecks = addCheck(Tchecks, 'MOMC-PRE-08', 'Optimization methodology contains TABLE_01_GA_configuration callout', ...
    contains(string(optSeg), 'TABLE_01_GA_configuration') || contains(string(txt), 'TABLE_01_GA_configuration'), ...
    sprintf('inOpt=%d inMaster=%d', contains(string(optSeg), 'TABLE_01_GA_configuration'), contains(string(txt), 'TABLE_01_GA_configuration')));
...
Tchecks = addCheck(Tchecks, 'MOMC-PRE-10', 'Required methodology anchors exist before completion', ...
    anchorsOK, join(anchorEvidence, ' | '));

% Preserve key Results section values exactly.
protectedResultsTokens = [
    "R1_solution_7"
    "R1_solution_3"
    "R1_solution_9"
    "Q_aux = 656.23 kWh"
...
resultsTokensOK = true;
resultsEvidence = strings(numel(protectedResultsTokens),1);

for k = 1:numel(protectedResultsTokens)
    c = countLiteral(resultsSegBefore, char(protectedResultsTokens(k)));
    resultsEvidence(k) = sprintf('%s=%d', protectedResultsTokens(k), c);
    if c == 0
        resultsTokensOK = false;
    end
end

Tchecks = addCheck(Tchecks, 'MOMC-PRE-11', 'Protected Results tokens exist before completion', ...
    resultsTokensOK, join(resultsEvidence, ' | '));
...
Tchecks = addCheck(Tchecks, 'MOMC-DRAFT-01', 'Draft contains methodology framing', ...
    contains(string(optBlock), 'optimization workflow') && contains(string(optBlock), 'multiobjective genetic-algorithm'), ...
    sprintf('workflow=%d GA=%d', contains(string(optBlock), 'optimization workflow'), contains(string(optBlock), 'multiobjective genetic-algorithm')));
...
Tchecks = addCheck(Tchecks, 'MOMC-DRAFT-11', 'Draft avoids citation placeholders', ...
    ~contains(string(optBlock), 'CITATION_NEEDED') && ~contains(string(optBlock), 'TODO') && ~contains(string(optBlock), 'TBD'), ...
...
% Ensure protected Results values were not altered.
resultsProtectedAfterOK = true;
resultsEvidenceAfter = strings(numel(protectedResultsTokens),1);

for k = 1:numel(protectedResultsTokens)
    c = countLiteral(resultsSegAfter, char(protectedResultsTokens(k)));
    resultsEvidenceAfter(k) = sprintf('%s=%d', protectedResultsTokens(k), c);
    if c == 0
        resultsProtectedAfterOK = false;
    end
end

Tchecks = addCheck(Tchecks, 'MOMC-POST-08', 'Protected Results tokens preserved after reconstruction', ...
    resultsProtectedAfterOK, join(resultsEvidenceAfter, ' | '));
