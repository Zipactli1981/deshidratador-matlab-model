...
function [X,F] = extract_XF_from_formal(formal)
    ...
    candidateX = ["X","x","population","Xformal","Xout"];
    candidateF = ["F","fval","Fformal","Fout","scores"];
    ...
    if isempty(F) && isstruct(formal)
        fns = fieldnames(formal);
        for i = 1:numel(fns)
            val = formal.(fns{i});
    ...
    if isempty(X) && isstruct(formal)
        fns = fieldnames(formal);
        for i = 1:numel(fns)
            val = formal.(fns{i});
            if isnumeric(val) && ismatrix(val) && size(val,2) == 4 && size(val,1) >= 1
                X = val;
                break
            end
        end
    ...
    if isempty(F)
        error('No se pudo extraer F del struct formal.');
    end
...
function [bestIdx,bestScore,H2like] = select_H2_like_solution(F)
    bestIdx = NaN;
    bestScore = NaN;
    H2like = false;

    if isempty(F) || size(F,2) < 3
        return
    end

    finiteRows = all(isfinite(F),2);
    admissible = finiteRows & F(:,1) < 0.1;
    ...
    Fn = (Fc - fmin)./denom;
    scores = sqrt(sum(Fn.^2,2));

    [bestScore,bLocal] = min(scores);
    bestIdx = candidates(bLocal);

    H2like = F(bestIdx,1) < 0.1 && ...
             F(bestIdx,2) < gasCost && ...
             F(bestIdx,3) < gasCO2;
end
