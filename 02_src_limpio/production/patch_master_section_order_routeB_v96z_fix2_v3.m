%% patch_master_section_order_routeB_v96z_fix2_v3.m
% 9.6z-fix2-script
...
nGluedAny = numel(regexp(newTxt, '[^\n#]#{1,6}\s+', 'match'));  % Avoid false positives on ##/### internal hash characters
...
prohibitedGlobalOpt = hasProhibitedClaim(newTxt, 'global\s+optimum|global\s+optimality|globally\s+optimal');
prohibitedPareto = hasProhibitedClaim(newTxt, 'global\s+Pareto\s+front|complete\s+Pareto[-\s]?front\s+characterization');
...
postGluedAny = numel(regexp(postTxt, '[^\n#]#{1,6}\s+', 'match'));
Tchecks = addCheck(Tchecks, 'FX2B-POST-03', 'No glued headings remain', postGluedAny == 0, sprintf('count=%d', postGluedAny));
...
function tf = hasProhibitedClaim(txt, pat)
    % Conservative claim detector. It flags prohibited wording only when it
    % appears outside explicit negation/restriction language. This prevents
    % false positives from manuscript-control text such as "does not claim",
    % "do not claim", or "instead of".
    lines = splitlines(string(normalizeNewlines(txt)));
    tf = false;
    for i = 1:numel(lines)
        li = strtrim(lines(i));
        if li == ""
        ...
        if isempty(regexpi(char(li), pat, 'once'))
            continue;
        end
        low = lower(char(li));
        allowed = contains(low, 'does not claim') || ...
            contains(low, 'do not claim') || ...
            contains(low, 'not claim') || ...
            contains(low, 'no claim') || ...
            contains(low, 'not as proof') || ...
            contains(low, 'not be interpreted') || ...
            contains(low, 'should not be interpreted') || ...
            contains(low, 'rather than') || ...
            contains(low, 'instead of') || ...
            contains(low, 'avoid') || ...
            contains(low, 'restriction') || ...
            contains(low, 'prohibited wording');
        if ~allowed
            tf = true;
            return;
        end
