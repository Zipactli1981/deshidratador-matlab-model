%% audit_master_manuscript_consistency_routeB_v96z.m
% 9.6z-audit-c-routeB
...
function present = hasUnsupportedGlobalOptimumClaim(txt)
    ...
    protective = [
        ...
        "not as proof of global optimality"
        "not as a proof of global optimality"
        "proof of global optimality"
        "no claim of global optimality"
        "should not be interpreted as a proof of global optimality"
    ];
    for i = 1:numel(protective)
        s = replace(s, protective(i), "");
    end
    present = contains(s,"global optimum") || contains(s,"globally optimal") || contains(s,"global optimality");
end

function present = hasUnsupportedGlobalParetoClaim(txt)
    s = lower(string(txt));
    protective = [
        "instead of global pareto front"
        "use computed nondominated set instead of global pareto front"
        "do not claim global pareto front"
        "no prohibited global pareto front wording"
        "instead of complete pareto-front characterization"
    ];
    for i = 1:numel(protective)
        s = replace(s, protective(i), "");
    end
    present = contains(s,"global pareto front") || contains(s,"complete pareto front") || contains(s,"complete pareto-front characterization");
end
