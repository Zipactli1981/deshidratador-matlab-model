...
%   - Solo crea runner y script de aprobación.
    ...
    checks{end+1,1} = check_row("RNG_D09","Approval gate returns command only",contains(approvalFileText,"approvedCommand") && contains(approvalFileText,"Only returns approved command."),"Approval stores approvedCommand as text; no runner call is executed inside approval gate.");
