%% export_master_manuscript_preliminary_review_v96z.m
% 9.6z-preview-a
...
banner = [
"<!--" newline ...
"PRELIMINARY REVIEW COPY" newline ...
"Generated: " datestr(now, 'yyyy-mm-dd HH:MM:SS') newline ...
"Source MASTER: " masterPath newline ...
"Scope: internal reading review, not pre-submission." newline ...
"Known open items: final source-verified references, in-text citations, style cleanup for repeated terms, final journal formatting." newline ...
"-->" newline newline ...
"# Preliminary review copy — MASTER manuscript v96z" newline newline ...
"> This copy is intended for full-content reading review. It is not a pre-submission version. The reference section remains a verification scaffold; in-text citations have not yet been finalized." newline newline ...
"---" newline newline ...
];

mdReview = char(string(banner) + string(txt));
...
"</style>" newline ...
    ...
    html = char("<!doctype html><html><head><meta charset=""utf-8""><title>MASTER manuscript v96z preliminary review</title>" + ...
        string(css) + "</head><body><main class=""page"">" + string(meta) + newline + ...
        strjoin(body, newline) + "</main></body></html>");
end
