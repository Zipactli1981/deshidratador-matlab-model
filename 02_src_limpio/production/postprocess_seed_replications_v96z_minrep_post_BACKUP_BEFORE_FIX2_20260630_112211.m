...
function [X,F] = extract_XF_from_formal(formal)
    ...
    % Primero: campos directos esperados.
    ...
    % Segundo: búsqueda recursiva en estructuras anidadas.
    if isempty(F)
        F = recursive_find_numeric_matrix(formal,3);
    end

    if isempty(X)
        X = recursive_find_numeric_matrix(formal,4);
    end

    % Tercero: si F viene como tabla, convertir.
    if istable(F)
    ...
    if isempty(F)
        error('No se pudo extraer F del struct formal ni por búsqueda recursiva.');
    end
...
function M = recursive_find_numeric_matrix(obj,nCols)
    M = [];

    if isnumeric(obj) && ismatrix(obj) && size(obj,2) == nCols && size(obj,1) >= 1
        M = obj;
        return
    end
    ...
    if istable(obj)
        try
            ...
            if isnumeric(A) && ismatrix(A) && size(A,2) == nCols && size(A,1) >= 1
                M = A;
                return
            end
        catch
    ...
    if isstruct(obj)
        ...
        % Priorizar nombres probables.
        preferred = ["F","fval","scores","X","x","population","result","results","output","formal"];
        ordered = strings(0,1);
        for p = 1:numel(preferred)
            hit = fns(strcmpi(fns,preferred(p)));
            if ~isempty(hit)
                ordered(end+1,1) = string(hit{1}); %#ok<AGROW>
            end
        end
        for q = 1:numel(fns)
            if ~any(ordered == string(fns{q}))
                ordered(end+1,1) = string(fns{q}); %#ok<AGROW>
            end
        end
        ...
        for i = 1:numel(ordered)
            try
                val = obj.(ordered(i));
                M = recursive_find_numeric_matrix(val,nCols);
                if ~isempty(M)
                    return
                end
            catch
    ...
    if iscell(obj)
        for i = 1:numel(obj)
            M = recursive_find_numeric_matrix(obj{i},nCols);
            if ~isempty(M)
                return
            end
        end
