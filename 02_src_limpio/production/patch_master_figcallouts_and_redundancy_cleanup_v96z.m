%% patch_master_figcallouts_and_redundancy_cleanup_v96z.m
...
T = addCheck(T,'FCRC-DRAFT-01','Computed nondominated set wording reduced below audit threshold',afterCount <= 10 && afterCount < beforeCount, ...
    sprintf('before=%d after=%d threshold<=10',beforeCount,afterCount));

H2 = detectHeadings(work);

figureRefs = countRegex(work,'\bFigure\s+[0-9]+');
T = addCheck(T,'FCRC-DRAFT-02','Figure references inserted',figureRefs >= 3, sprintf('figureRefs=%d',figureRefs));

T = addCheck(T,'FCRC-DRAFT-03','Figure 1 system schematic callout present',contains(string(work),'Figure 1') && contains(string(work),'system schematic'), ...
    sprintf('Figure1=%d systemSchematic=%d',contains(string(work),'Figure 1'),contains(string(work),'system schematic')));

T = addCheck(T,'FCRC-DRAFT-04','Figure 2 optimization workflow callout present',contains(string(work),'Figure 2') && contains(string(work),'optimization workflow'), ...
    sprintf('Figure2=%d workflow=%d',contains(string(work),'Figure 2'),contains(string(work),'optimization workflow')));

T = addCheck(T,'FCRC-DRAFT-05','Figure 3 selected candidates/trade-off callout present',contains(string(work),'Figure 3') && contains(string(work),'R1_solution_7') && contains(string(work),'Q_aux'), ...
...
hard = ["FCRC-DRAFT-01","FCRC-DRAFT-02","FCRC-DRAFT-03","FCRC-DRAFT-04","FCRC-DRAFT-05","FCRC-POST-01","FCRC-POST-02","FCRC-POST-03","FCRC-POST-04","FCRC-POST-05"];
