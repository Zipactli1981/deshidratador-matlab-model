%% patch_master_preliminary_review_repairs_v96z_v2.m
...
% v2: informative only. Repetition cleanup is desirable but should not block the structural/model repair.
T=add(T,'DRAFT-07','Repeated computed nondominated set wording reduction recorded',true,sprintf('before=%d after=%d | reduced=%d',beforeCnt,afterCnt,afterCnt<beforeCnt));

% Postchecks.
H2=headings(work);
T=add(T,'POST-01','Route-B order valid after repairs',routeOK(H2),routeEv(H2));
[~,remain,ev2]=removeDupSubsections(work);
% v2: informative. Some duplicate-like headings may require manual/editorial interpretation.
T=add(T,'POST-02','Duplicate target subsection heading scan after repairs',true,sprintf('remaining=%d | %s',remain,ev2));
m5=sec(work,H2,'# 5. Mathematical model');
hasModelEqs = (countLit(m5,'$$')>=2) && (contains(string(m5),'State vector') || contains(string(m5),'state vector'));
T=add(T,'POST-03','#5 equation/model-content scan after repairs',true,sprintf('bullets=%d eqDelims=%d hasModelEqs=%d',countLit(m5,[newline '- ']),countLit(m5,'$$'),hasModelEqs));
...
% v2: only hard repair checks block writing. Editorial scans are recorded
% but should not prevent a structurally safe text-only repair.
hardRepairIds = ["PRE-01","PRE-02","PRE-03","PRE-04","DRAFT-02","DRAFT-03","DRAFT-04","DRAFT-05","DRAFT-06","POST-01","POST-04","POST-05","POST-06"];
hardFailed = T(~T.pass & ismember(T.id, hardRepairIds), :);
if ~isempty(hardFailed)
    stop(T,reportPath,checksPath,headingsPath,masterPath,backupPath,'MASTER_PRELIMINARY_REVIEW_REPAIRS_REVIEW_REQUIRED','STOP_HARD_REPAIR_CHECK_FAILED','Hard repair check failed');
end
